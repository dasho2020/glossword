	...
	case 'localfile':
		if ( isset( $_FILES['arp'] ) )
		{
			...
			if ( move_uploaded_file( $_FILES['arp']['tmp_name']['localfile'], $filename_temp ) )
			{
				...
				/* Read file */
				$ar['input'] = file_get_contents( $filename_temp );
				/* Delete a temporary file */
				@unlink( $filename_temp );
			}
		}
...
/* Language ID */
/* 1.9.4: Get the first language in the list of activated languages */
$id_lang = isset( $ar['id_lang'] ) ? $ar['id_lang'] : current( $this->o->ar_languages_locale );
			...
			foreach ( $oXml->entry as $entry )
			{
				...
				foreach ( $entry->children() as $o_child )
				{
					...
					if ( $id_field  )
					{
						...
						/* 1.9.3: Change an existent value for only installed languages */
						if ( $xml_lang )
						{
				...
				foreach ( $q__contents as $id_fields => &$q__ar_v )
				{
					foreach ( $q__ar_v as $int_contents => &$ar_v )
					{
						...
						else if ( !isset( $q__contents_si[$ar_v['id_contents']] ) )
						{
							...
							$oSearchIndex->add_contents( $ar_v['id_contents'], $id_item_max, $id_lang, $q__ar_v[$int_contents]['_si'] );
						}
